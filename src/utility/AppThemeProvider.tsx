...
export default function AppThemeProvider({ children }: Props) {
  ...
  const [mode, setMode] = useState<"light" | "dark">(
    prefersDarkMode && siteConfig.darkMode ? "dark" : "light",
  );
  ...
  const changeSystemThemeListener = useCallback((e: MediaQueryListEvent) => {
    if (
      store.getState().theme.colorScheme === colorSchemes.system &&
      siteConfig.darkMode
    ) {
  ...
  useEffect(() => {
    ...
    return () =>
      darkThemeSystem.removeEventListener("change", changeSystemThemeListener);
